...
# @param ... Additional arguments passes to use_function
# @return A named list
make_assoc_parts3 <- function(use_function = make_assoc_parts_for_stan, 
                             newdata, assoc, grp_stuff, meta_stuff, 
                             ids, times, id_var, time_var, object, ...) {
  
  if (!requireNamespace("data.table"))
    stop("the 'data.table' package must be installed to use this function")
  
  epsilon    <- object$epsilon
  auc_qnodes <- object$auc_qnodes
  ...
  dataQ <- rolling_merge(data = newdata, ids = as.factor(ids), times = times, grps = grps)
  dataQ[[id_var]] <- as.integer(as.factor(dataQ[[id_var]] ))

  mod_eta <- use_function(newdata = dataQ, object, ...)
